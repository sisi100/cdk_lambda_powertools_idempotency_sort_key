...
class CustomPersistenceLayer(BasePersistenceLayer):
    def __init__(
        ...
        table_name: str,
        key_attr: str = "id",
        sort_key_attr: str = "sk",
        expiry_attr: str = "expiration",
        status_attr: str = "status",
        sort_key_value: str = "dummy",
        data_attr: str = "data",
    ...
    def _get_record(self, idempotency_key) -> DataRecord:
        response = self.table.get_item(
            Key={self.key_attr: idempotency_key, self.sort_key_attr: self.sort_key_value}, ConsistentRead=True
        )
    ...
    def _put_record(self, data_record: DataRecord) -> None:
        item = {
            ...
            self.sort_key_attr: self.sort_key_value,
        ...
        try:
            self.table.put_item(
                Item=item,
                ConditionExpression=f"attribute_not_exists({self.key_attr}) AND attribute_not_exists({self.sort_key_attr}) OR {self.expiry_attr} < :now",
    ...
    def _update_record(self, data_record: DataRecord):
        ...
        kwargs = {
            "Key": {self.key_attr: data_record.idempotency_key, self.sort_key_attr: self.sort_key_value},
